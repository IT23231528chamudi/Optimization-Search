...
def feasible_vertices(constraints: List[Constraint]) -> List[Tuple[float, float]]:
    ...
    # Add non-negativity as <= constraints: -x <= 0 and -y <= 0
    cons.append((-1.0, 0.0, 0.0))
    cons.append((0.0, -1.0, 0.0))

    cand: List[Tuple[float, float]] = [(0.0, 0.0)]
    m = len(cons)
    for i in range(m):
        for j in range(i + 1, m):
            p = _intersect(cons[i], cons[j])
            if p is not None:
                cand.append(p)

    # Filter feasible and de-duplicate
...
def maximize_objective(vertices: List[Tuple[float, float]], c1: float, c2: float) -> Tuple[Tuple[float, float], float]:
    ...
    for x, y in vertices[1:]:
        ...
        if val - best_val > EPS:
            ...
            # deterministic tie-breaker: prefer larger x, then larger y
...
def knapsack_bottom_up(values: List[int], weights: List[int], capacity: int) -> int:
    ...
    if n != len(weights) or capacity < 0:
        return 0
    # Option A: dp[i][cap] = best using items i..n-1
    ...
    for i in range(n - 1, -1, -1):
        ...
        for cap in range(0, capacity + 1):
            ...
            take = -1
            if w_i <= cap:
                take = v_i + dp[i + 1][cap - w_i]
            dp[i][cap] = skip if take < 0 else max(skip, take)
...
def knapsack_top_down(values: List[int], weights: List[int], capacity: int) -> int:
    ...
    def f(i: int, cap: int) -> int:
        ...
        return skip if take < skip else take
